...
projectUID - GitHub Project V2 UID (from projmng.yaml projman.uid) - setter only
statusFieldUID - Status field UID (from projmng.yaml projman.status_field_uid) - setter only
itemUID - (optional) Project item UID from projmng.yaml active_item.item_uid - if provided, skip fetching
...
# Use cached stage option UID from projmng.yaml or constant from note below
...
# Set the stage using cached project UID and status field UID from projmng.yaml
gh api graphql -f query='
